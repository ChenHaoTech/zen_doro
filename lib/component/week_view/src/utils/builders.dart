...
class DefaultBuilders {
  ...
  static Widget defaultEventTextBuilder(FlutterWeekViewEvent event, BuildContext context, DayView dayView, double height, double width) {
    ...
    while (exceedHeight ?? true) {
      exceedHeight = $exceedHeight(text, event.textStyle, height, width);
  ...
  static Widget defaultCurrentTimeIndicatorBuilder(
    ...
    return Positioned(
      top: topOffsetCalculator(customerHourMinute ?? HourMinute.now()) - timeIndicatorHight / 2,
      left: isRtl ? 0 : hoursColumnWidth,
      right: isRtl ? hoursColumnWidth : 0,
      child: decorator?.call(row) ?? row,
    );
  }
  ...
  static bool? $exceedHeight(List<TextSpan> input, TextStyle? textStyle, double height, double width) {
  ...
  static bool ellipsize(List<TextSpan> input, [String ellipse = '…']) {
    if (input.isEmpty) {
      return false;
    }
