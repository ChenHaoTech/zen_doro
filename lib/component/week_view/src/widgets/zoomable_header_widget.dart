...
import 'package:flutter_pasteboard/component/week_view/src/styles/drag_and_drop.dart';
import 'package:flutter_pasteboard/component/week_view/src/styles/hours_column.dart';
import 'package:flutter_pasteboard/component/week_view/src/styles/resize_event.dart';
import 'package:flutter_pasteboard/component/week_view/src/styles/zoomable_header_widget.dart';
...
typedef BackgroundTapCallback = Function(DateTime date, TapUpDetails details);
typedef BackgroundDragCallback = Function(DateTime date);
typedef BackgroundDragStartCallback = Function(DateTime date);
...
abstract class ZoomableHeadersWidget<S extends ZoomableHeaderWidgetStyle, C extends ZoomController> extends StatefulWidget {
  ...
  final BackgroundDragCallback? onBackgroundDragUpdate;
  final BackgroundDragStartCallback? onBackgroundDragStart;
...
abstract class ZoomableHeadersWidgetState<W extends ZoomableHeadersWidget> extends State<W> with ZoomControllerListener {
  ...
  void initState() {
    super.initState();
    hourRowHeight = calculateHourRowHeight();
  ...
  void onZoomFactorChanged(ZoomController controller, ScaleUpdateDetails details) {
    // DebugUtils.log("zoomable_header_widget:168: ${this.debugKey} \n${StackTrace.current}");
    if (!mounted) {
      return;
    }

    double hourRowHeight = calculateHourRowHeight(controller);
    ...
    // DebugUtils.log("zoomable_header_widget:178: ${this.debugKey}, ${hourRowHeight} \n${StackTrace.current}");
    setState(() {
  ...
  double calculateTopOffset(HourMinute time, {HourMinute? minimumTime, double? hourRowHeight}) => DefaultBuilders.defaultTopOffsetCalculator(time,
  ...
  double calculateHourRowHeight([ZoomController? controller]) => currentDayViewStyle.hourRowHeight * (controller ?? widget.controller).zoomFactor;
}