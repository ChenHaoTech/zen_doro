...
class DayBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: DayBarWidgetSupplier.mayBeof(context)?.barSupplier.call(date, height, width) ??
            Container(
              height: height,
              width: width,
...
class DayBarWidgetSupplier extends InheritedWidget {
  final Widget Function(DateTime date, double? height, double? width) barSupplier;

  const DayBarWidgetSupplier({
    super.key,
    required Widget child,
    required this.barSupplier,
  }) : super(child: child);

  static DayBarWidgetSupplier? mayBeof(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<DayBarWidgetSupplier>();
  }

  @override
  bool updateShouldNotify(DayBarWidgetSupplier old) {
    return true;
  }
}
