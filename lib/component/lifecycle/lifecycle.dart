...
class Lifecycle with WidgetsBindingObserver {
  ...
  bindImplIntoRoute(LifecycleMixin lifecycleImpl) {
    ...
    // if (b) this.log.e("Lifecycle bindImplIntoRote error: this key is already bound");
    // // if (b)
    // //   throw Exception(
  ...
  replaceRoute(Route? newRoute, Route? oldRoute) {
    ...
    _routeQueue = Queue.from(_routeQueue.map((e) => e == oldRoute ? newRoute : e));
  }
  ...
  _doShow() {
    printLog(content: "_map.length:${_map.length}", title: LifeNavigatorObserver.TAG);
    if (_routeQueue.isEmpty) return;
    _map.forEach((key, value) {
