...
class HorizontalWeekCalendar extends StatefulWidget {
  ...
  @override
  State<HorizontalWeekCalendar> createState() => HorizontalWeekCalendarState();
}

class HorizontalWeekCalendarState extends State<HorizontalWeekCalendar> {
                              ...
                              Builder(builder: (_) {
                                ...
                                return Expanded(
                                  child: GestureDetector(
                                    ...
                                    // },
                                    // TODO: disabled
                                    onTap: _isReachMaximum(currentDate) && _isReachMinimum(currentDate)
                                        ? () {
                                            _onDateSelect(
                                    ...
                                    child: Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        color: _computeBgColor(currentDate, theme),
                                        border: Border.all(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Text(
                                            ...
                                            style: theme.textTheme.titleMedium!.copyWith(
                                              color: _computeColor(currentDate),
                                              fontWeight: FontWeight.bold,
                                            ),
                                          ),
                                          Text(
  ...
  Color _computeBgColor(DateTime currentDate, ThemeData theme) {
    var dateFormat = DateFormat('dd-MM-yyyy');
    var today = dateFormat.format(currentDate) == dateFormat.format(DateTime.now());
    var selected = dateFormat.format(currentDate) == dateFormat.format(selectedDate);

    if (!selected && today) {
      return (widget.activeBackgroundColor ?? theme.primaryColor).withOpacity(.8);
    }
    return selected
  ...
  Color _computeColor(DateTime currentDate) {
    var dateFormat = DateFormat('dd-MM-yyyy');
    var selected = dateFormat.format(currentDate) == dateFormat.format(selectedDate);
    var today = dateFormat.format(currentDate) == dateFormat.format(DateTime.now());
    if (!selected && today) {
