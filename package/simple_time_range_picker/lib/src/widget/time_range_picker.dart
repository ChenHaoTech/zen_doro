...
class _TimeRangeDialogState extends State<_TimeRangeDialog> with SingleTickerProviderStateMixin {
  ...
  final double _kTimePickerHeightPortrait = 530.0;
  final double _kTimePickerHeightLandscape = kIsWeb ? 350 : 316.0;

  @override
  void initState() {
    super.initState();
    _tabController = TabController(vsync: this, length: 2, initialIndex: widget.timeRangeViewType == TimeRangeViewType.start ? 0 : 1);
    _startTime = widget.startTime ?? null;
    _endTime = widget.endTime ?? null;
    _startDefaultTime = _startTime ?? (widget.unSelectedEmpty ? null : TimeOfDay.now());
    _endDefaultTime = _endTime ?? (widget.unSelectedEmpty ? null : TimeOfDay.now());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
        ...
        content: Container(
          ...
          width: _orientation == Orientation.portrait ? _kTimePickerWidthPortrait : _kTimePickerWidthLandscape,
          height: _orientation == Orientation.portrait ? _kTimePickerHeightPortrait : _kTimePickerHeightLandscape,
          child: Scaffold(
            appBar: TabBar(labelColor: Theme.of(context).textTheme.bodyLarge!.color, controller: _tabController, tabs: [
              Tab(text: _formatTime(_startTime) ?? widget.headerDefaultStartLabel),
  ...
  Widget _picker(TimeOfDay? initialTime, ValueChanged<TimeOfDay> onTimeChange) {
    return TimeSinglePicker(
      initialTime: initialTime ?? TimeOfDay.now(),
      cancelText: widget.cancelLabel,
      confirmText: widget.okLabel,
      onTimeChange: onTimeChange,
      onSubmitted: (value) =>
          widget.onSubmitted?.call(TimeRangeValue.value(startTime: _startTime ?? _startDefaultTime, endTime: _endTime ?? _endDefaultTime)),
      onCancel: widget.onCancel,
    );
    return SingleChildScrollView(
      child: TimeSinglePicker(
        initialTime: initialTime ?? TimeOfDay.now(),
        cancelText: widget.cancelLabel,
        confirmText: widget.okLabel,
        onTimeChange: onTimeChange,
