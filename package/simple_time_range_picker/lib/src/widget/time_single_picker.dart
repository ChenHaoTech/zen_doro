...
const double _kTimePickerHeaderTextScaleFactor = 1;
...
const double _kTimePickerHeightPortrait = 386.0;
      ...
      case Orientation.portrait:
        ...
        controls = Column(
          children: <Widget>[
            ...
            Container(
              height: _kTimePickerHeaderControlHeight, //kMinInteractiveDimension * 2,
              child: Row(
                children: <Widget>[
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  _TappableLabel _buildTappableLabel(TextTheme textTheme, Color? color, int value, String label, VoidCallback onTap) {
    final TextStyle style = textTheme.titleMedium!.copyWith(color: color);
...
/// [Navigator.pop].
class TimeSinglePicker extends StatefulWidget {
  /// Creates a material time picker.
  ///
  /// [initialTime] must not be null.
  const TimeSinglePicker({
    Key? key,
  ...
  @override
  _TimeSinglePickerState createState() => _TimeSinglePickerState();
}

class _TimeSinglePickerState extends State<TimeSinglePicker> {
  ...
  void _handleOk() {
    ...
    widget.onSubmitted(_selectedTime);
      ...
      case TimePickerEntryMode.dial:
        switch (orientation) {
          case Orientation.portrait:
            ...
            timePickerHeight =
                theme.materialTapTargetSize == MaterialTapTargetSize.padded ? _kTimePickerHeightPortrait : _kTimePickerHeightPortraitCollapsed;
            break;
          case Orientation.landscape:
            timePickerWidth = _kTimePickerWidthLandscape * textScaleFactor;
            timePickerHeight =
                theme.materialTapTargetSize == MaterialTapTargetSize.padded ? _kTimePickerHeightLandscape : _kTimePickerHeightLandscapeCollapsed;
            break;
        }
        break;
  ...
  Widget build(BuildContext context) {
    ...
    switch (_entryMode) {
      case TimePickerEntryMode.dial:
        final Widget dial = ExcludeSemantics(
          child: AspectRatio(
            aspectRatio: 1.0,
            child: _Dial(
              mode: _mode,
    ...
    return Dialog(
      ...
      insetPadding: EdgeInsets.symmetric(
        horizontal: 0.0,
        vertical: _entryMode == TimePickerEntryMode.input ? 0.0 : 0.0,
      ),
