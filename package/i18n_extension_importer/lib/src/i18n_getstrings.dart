...
class ExtractedString extends Equatable {
  ...
  ExtractedString(this.string, this.lineNumber, {this.pluralRequired = false, this.sourceFile = ""});
...
class DecodedSyntax {
  ...
  DecodedSyntax.valid(List<I18nRequiredModifiers> modifiers) : this._(valid: true, modifiers: modifiers);
...
class StringExtractor extends UnifyingAstVisitor<void> {
  ...
  void _handleI18nSyntax(DecodedSyntax syntax, StringLiteral node) {
    if (syntax.valid && node.stringValue != null) {
      ...
      final ExtractedString s =
          ExtractedString(node.stringValue!, lineNo, pluralRequired: syntax.modifiers.contains(I18nRequiredModifiers.plural), sourceFile: fileName);
      strings.add(s);
    }
  }
  ...
  DecodedSyntax _hasI18nSyntax(AstNode self, AstNode parent) {
    ...
    while (here != null && here != parent.endToken) {
      if (here == self.beginToken && here.next!.type.lexeme == "." && suffixes.contains(here.next!.next!.value())) {
